...
#Function that adds all of the columns to display the assignment averages
print("Assigment Averages")
for column in range(len(gradebook[1])):
    #iterate over the columns
    assignment_scores = []
    for row in range(len(gradebook)):
        assignment_scores.append((gradebook[row][column]))
    print(f"Assignment {column+1}: {sum(assignment_scores) / len(assignment_scores):.2f}")

print("\nStudent Averages:")
for row in range(len(gradebook)):
    print(f"Student {row + 1}: {sum(gradebook[row]) / len(gradebook[row]):.2f}")
